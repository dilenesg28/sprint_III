...
export default function CardFilme({filme}){
    ...
    function favoritar(){
        ...
        const options = {
          ...
          fetch('https://api.themoviedb.org/3/account/20502027/watching', options)
    ...
    function desfavoritar(){
        ...
        const options = {
            ...
            body: JSON.stringify({media_type: 'movie', media_id: filme.id, watching: false})
          };
    ...
    return (
        <div className="flex flex-col items-center justify-between gap-1 w-40 m-2 relative">
            { favorito ? 
                <HeartIcon 
                    className="h-6 w-6 text-yellow-600 absolute top-1 right-2 cursor-pointer"
                    onClick={desfavoritar}
                 />
                :
                <HeartIconOutline 
                    className="h-6 w-6 absolute top-1 right-2 cursor-pointer hover:text-yellow-600" 
                    onClick={favoritar}
                />
            }
            ...
            <a href="#" className="bg-green-600 w-full rounded text-center py-1 hover:bg-red-900">
                detalhes
            </a>
        </div>
    )
